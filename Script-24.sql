...
select product_id, count(*) as total_sales from sales
group by product_id;

select * from sales;
-- 11. Write a query to find the total quantity sold for each product from the `Sales` table.
select product_id, SUM(quantity_sold) as total_quantity from sales
group by product_id;
...
select concat(first_name,' ',last_name), total_amount
from customers c
join sales s
on c.customer_id = s.customer_id;
...
set search_path to assignement;
show search_path;